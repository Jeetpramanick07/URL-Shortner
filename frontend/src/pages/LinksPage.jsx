...
import { linksApi } from '../api/services'
import { EmptyState, GlassCard, PageHeader, StatusBadge, Toast } from '../components/ui'
  ...
  return (
    <>
      <PageHeader
        ...
        title="All Smart Links"
        description="Manage and monitor your Amazon short links."
        actions={<><button className="button glass" onClick={load}><RefreshCw size={17} className={loading ? 'spin' : ''} /> Refresh</button><Link className="button primary" to="/links/new"><Plus size={17} /> Create Link</Link></>}
      />

      <GlassCard className="filters-card">
        <div className="filter-search"><Search size={17} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search slug, ASIN, or domain" /></div>
        <label><span>Marketplace</span><select value={marketplace} onChange={(e) => setMarketplace(e.target.value)}><option value="all">All marketplaces</option><option value="IN">Amazon IN</option><option value="US">Amazon US</option><option value="UK">Amazon UK</option><option value="DE">Amazon DE</option></select></label>
        <label><span>Status</span><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">All statuses</option><option value="active">Active</option><option value="disabled">Disabled</option></select></label>
        <label><span>Date created</span><select><option>Last 30 days</option><option>Last 90 days</option><option>All time</option></select></label>
      </GlassCard>

      <GlassCard className="table-card links-table-card">
        {filtered.length ? (
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td>{Number(item.approximate_unique_visitors ?? Math.round(Number(item.total_clicks || 0) * .7)).toLocaleString()}</td>
      ...
      <div className="mini-stats-row">
        <GlassCard><span>Total clicks</span><strong>{links.reduce((sum, item) => sum + Number(item.total_clicks ?? item.click_sequence ?? 0), 0).toLocaleString()}</strong></GlassCard>
        <GlassCard><span>Average CTR</span><strong>8.4%</strong></GlassCard>
        <GlassCard><span>Active links</span><strong>{links.filter((item) => item.is_active).length}</strong></GlassCard>
        <GlassCard><span>API calls</span><strong>42K</strong></GlassCard>
      </div>
