...
This backend is FastAPI/Python, not Node.js/Express. Import the repository as a Vercel project and set its **Root Directory** to `backend`. Vercel detects `app/main.py` as the FastAPI entry point. Leave Build Command and Output Directory unset.

Configure the production environment variables listed in `.env.example`. Use `APP_ENV=production`, `DEBUG=false`, `PUBLIC_SCHEME=https`, and strong independent values for `ADMIN_API_KEY` and `IP_HASH_SECRET`. `DATABASE_URL` must point to an externally hosted PostgreSQL database and use SQLAlchemy's `postgresql+psycopg://` scheme. Set `CORS_ORIGINS` to the exact HTTPS origin of the deployed frontend.

Apply the schema from a trusted local or CI environment before serving traffic; do not run migrations during a Vercel build:
...
Attach the API hostname and every public short-link hostname to the backend Vercel project. Include those short-link hostnames in `SEED_DOMAINS` before running the seed command, or create them through `POST /api/domains`.
